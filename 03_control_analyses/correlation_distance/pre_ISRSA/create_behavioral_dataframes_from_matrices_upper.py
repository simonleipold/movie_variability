#!/usr/bin/env python
## this script creates dataframes from the full distance matrices for Features (pre- and post-session),
## and Naming (pre- and post-session) and saves them to a new CSV file
...
matrix_dir = Path('/project/3011157.03/Simon/proj_2022_CABB_movie/Scripts/MovVar_CommsBio_Revision01/control_corrdis/matrices')

# Directory to save the results
output_dir = Path('/project/3011157.03/Simon/proj_2022_CABB_movie/Scripts/MovVar_CommsBio_Revision01/control_corrdis/dfs_behavior')
if not output_dir.exists():
    output_dir.mkdir(exist_ok=True)
    print(f'Created directory: {output_dir}')

# Iterate over each task and session
for task in ['Features', 'Naming']:
    for session in ['pre', 'post']:
        # Form the file name according to the provided pattern
        matrix_filename = matrix_dir / f'{task}_{session}.csv'
        if matrix_filename.exists():
            ...
            output_file_name = output_dir / f'{task}_{session}_df_upper.csv'
