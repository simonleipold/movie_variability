#!/usr/bin/env python3
## this script creates dataframes from the full distance matrices
## for each movie and parcel (HCP 7T version)
...
all_pairs_list_path = data_dir / 'all_pair_list_with_reverse_hcp7T.csv'
...
# match naming from your original IS-RSA pipeline
output_dir = (
    projpath
    / 'Scripts'
    / 'MovVar_ImagNeuro_Revision01'
...
for movie in range(1, num_movies + 1):
    for node in range(1, num_nodes + 1):
        ...
        if matrix_filename.exists():
            ...
            distance_matrix = pd.read_csv(matrix_filename, index_col=0, dtype=str)
