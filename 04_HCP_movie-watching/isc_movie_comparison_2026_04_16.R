...
movies <- paste0("movie", 1:14) # changed from 8 to 14 to match HCP7T data
...
get_disjoint_pairs <- function(data, n_pairs = 89, seed = 123) {
  set.seed(seed)
...
selected_pairs <- get_disjoint_pairs(combined_tibble, 89)
...
output_file_path <- file.path(outdir, "isc_anova_HCP.csv")
...
# Define the path for the output file
output_file_path <- file.path(outdir, "Figure5C.csv")
