...
in_path = os.path.join(proj_path, 'Scripts', 'MovVar_ImagNeuro_Revision01','HCP_movie-watching', 'r_output_anova')
## location where visualizations and niftis will be stored
out_dir = os.path.join(proj_path, 'Scripts', 'MovVar_ImagNeuro_Revision01','HCP_movie-watching', 'visualizations')
...
display = nplot.plot_glass_brain(threshold(beta_brain, pval_brain, thr=0.05).to_nifti(),
    colorbar = True, plot_abs = False, cmap = "plasma", vmin = 0, vmax = 40, threshold=1e-12
    )
