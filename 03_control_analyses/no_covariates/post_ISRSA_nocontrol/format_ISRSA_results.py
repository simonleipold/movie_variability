...
r_path = os.path.join(proj_path, 'Scripts', 'MovVar_CommsBio_Revision01', 'control_ISRSA', 'r_output_nocontrol')
## location where visualizations and niftis will be stored
out_dir = os.path.join(proj_path, 'Scripts', 'MovVar_CommsBio_Revision01', 'control_ISRSA', 'r_output_nocontrol','formatted')
if not os.path.exists(out_dir):
    os.makedirs(out_dir, exist_ok=True)
    print('Directory %s created' % out_dir)

## create a task list for plots and files
task_flist = ['ISRSA_features_post_', 'ISRSA_features_pre_']
